...
import { TravelRegionMap, RegionMapChoices } from '@/src/components/TravelRegionMap';
...
export default function TravelMapScreen() {
  ...
  const selectArea = (code: string) => { setSelectedAreaCode(code); setSelectedSigunguCode(''); };
  ...
  const selectedAreaRecords = useMemo(() => records.filter((record) => record.areaCode === selectedArea.code).sort((a, b) => b.startDate.localeCompare(a.startDate)), [records, selectedArea.code]);
  ...
  const visitedAreaCount = REGIONS.filter(area => (areaCounts[area.code] ?? 0) > 0).length;
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      <ScrollView onLayout={event => setLayoutWidth(event.nativeEvent.layout.width)} style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.intro}>
          <Text style={styles.introEyebrow}>대한민국 지역별 여행지도</Text>
          <Text style={styles.introTitle}>여행이 쌓이면, 지도가 물들어요.</Text>
          <Text style={styles.introDescription}>발걸음이 닿은 지역과 그곳에서 남긴 이야기를 한눈에.</Text>
        </View>
        <View style={[styles.mapLayout, layoutWidth >= 800 && styles.mapLayoutWide]}>
          <TravelRegionMap counts={areaCounts} selectedCode={selectedArea.code} onSelect={selectArea} />
          <View style={[styles.mapAside, layoutWidth >= 800 && styles.mapAsideWide]}>
            <View style={styles.atlasStats}>
              <View><Text style={styles.atlasLabel}>방문한 시·도 / 전체</Text><Text style={styles.atlasNumber}>{visitedAreaCount}<Text style={styles.atlasTotal}> / {REGIONS.length}</Text></Text></View>
              <View><Text style={styles.atlasLabel}>총 여행 기록</Text><Text style={styles.atlasNumber}>{records.length}<Text style={styles.atlasTotal}>개</Text></Text></View>
            </View>
            <View style={styles.selectedPanel} accessibilityLiveRegion="polite">
              <Text style={styles.selectedEyebrow}>선택한 지역</Text>
              <Text style={styles.selectedName}>{selectedArea.name}</Text>
              <Text style={styles.selectedCount}>여행 기록 {areaCounts[selectedArea.code] ?? 0}개</Text>
              <Text style={styles.selectedHint}>{selectedAreaRecords.length ? '아래에서 상세 지역과 여행 이야기를 확인하세요.' : '이 지역에 첫 여행 기록을 남겨보세요.'}</Text>
            </View>
            <Text style={styles.areaListTitle}>지역별 기록</Text>
            <RegionMapChoices counts={areaCounts} selectedCode={selectedArea.code} onSelect={selectArea} />
          </View>
        </View>
